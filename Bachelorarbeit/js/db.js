...
function checkBenutzer(page) {
  var benutzername = "";

  if (page == "registrieren") {
    benutzername = document.getElementById("Rbenutzername").value;
  } else if (page == "pVergessen") {
    benutzername = document.getElementById("Vbenutzername").value;
  }

  if (benutzername != "") {
    $.ajax({
      url: 'php/check-benutzer.php',
      ...
      success: function (data) {
        ...
        if (array.length != 0) {
          if (page == "registrieren") {
            console.log("Benutzername schon belegt");
            showFehler("Benutzername ist nicht verfügbar");
            console.log(array[0].Id_Benutzer);
          } else if (page == "pVergessen") {
            console.log("Benutzername vorhanden um Passwort zu ändern");
            checkPasswort(page);
          }
        } else {
          if (page == "registrieren") {
            // REGISTRIERUNG - Neuen Benutzer anlegen
            console.log("Benutzername verfügbar");
            checkPasswort(page);
          } else if (page == "pVergessen") {
            // PASSWORT VERGESSEN - 
            console.log("Benutzername nicht gefunden");
            showFehler("Benutzername nicht gefunden");
          }
        }
      }
    });
  } else {
    showFehler("Benutzername eingeben");
  }
}

function checkPasswort(page) {
  var benutzername = "";
  var passwort = "";
  var passwortWiderholung = "";

  if (page == "registrieren") {
    benutzername = document.getElementById("Rbenutzername").value;
    passwort = document.getElementById("Rpasswort").value;
    passwortWiderholung = document.getElementById("RpasswortW").value;
  } else if (page == "pVergessen") {
    benutzername = document.getElementById("Vbenutzername").value;
    passwort = document.getElementById("Vpasswort").value;
    passwortWiderholung = document.getElementById("VpasswortW").value;
  }

  if (passwort.length > 6) {
    if (passwort == passwortWiderholung && page == "registrieren") {
      $.ajax({
        url: 'php/add-benutzer.php',
        data: 'benutzername=' + benutzername + '&passwort=' + passwort,
        type: "POST",
        success: function (data) {
          console.log("Benutzer erfolgreich angelegt");
          window.location.href = "./registrieren_erfolgreich.html";
        }
      });
    } else if (passwort == passwortWiderholung && page == "pVergessen") {
      $.ajax({
        url: 'php/update-benutzer.php',
        data: 'benutzername=' + benutzername + '&passwort=' + passwort,
        type: "POST",
        success: function (data) {
          console.log("Passwort erfolgreich geändert");
          window.location.href = "./passwort_vergessen_geändert.html";
        }
      });
    } else {
      showFehler("Passwort stimmt nicht überein");
    }
  } else {
    showFehler("Passwort zu kurz");
  }
}
